...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'hvi:k:d:',
    ...
    for gpdm in zxglb :
        ...
        try :
            gpmc = gpdmdict()[gpdm]
            gpeps = getttmeps(gpdm,hgall,zzgball,epsall)
        except:
            continue
        print('共有%d只股票，正在处理第%d只股票：%s%s，请等待。' % (len(zxglb),i,gpdm,gpmc))
        genimg(gpdm,gpeps,imgdir)
...
def genimg(gpdm,eps,imgdir):
    ...
    try :
        gjsydf = pd.merge(gj,eps,left_index = True, right_index = True)
        gjsydf.eval('pe_ttm=adj_close/eps_ttm')
    except :
        return

    csvfn = imgdir+'\\'+gpdmdict()[gpdm].replace(' ','').replace('*','')+'.csv'
    gjsydf.to_csv(csvfn)
    ...
    ax1.plot(gjsydf.index,gjsydf['pe_ttm'],color="blue",linewidth=1.5,label='pe(ttm)')
    ax2.plot(gjsydf.index,gjsydf['adj_close'],color="red",linewidth=1.5,label='adj_close')
    ax3.plot(gjsydf.index,gjsydf['eps_ttm'],color="green",linewidth=1.5,label='eps(ttm)')

    title = gpdm+ gpdmdict()[gpdm] + "股价、市盈率及每股收益走势图"
    ...
    ax1.set_ylim(0,min(200,max(gjsydf['pe_ttm'].fillna(0))))
    ...
    imgfn = imgdir+'\\'+gpdmdict()[gpdm].replace(' ','').replace('*','')+'.png'
    plt.savefig(imgfn)
