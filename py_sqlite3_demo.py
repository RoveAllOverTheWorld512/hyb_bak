# -*- coding: utf-8 -*-
"""
Created on Sun Nov 26 14:12:42 2017
...
数据库文件，而是直接打开该数据库文件。
    ...
    创建在硬盘上面： conn = sqlite3.connect('D:\\SQLite\\test.db')
    创建在内存上面： conn = sqlite3.connect('"memory:')

    下面我们一硬盘上面创建数据库文件为例来具体说明：
    conn = sqlite3.connect('D:\\SQLite\\hongten.db')
...
def save_test():
    '''保存数据测试...'''
    print('保存数据测试...')
    save_sql = '''INSERT OR IGNORE INTO student values (?, ?, ?, ?, ?, ?)'''
...
def init():
    ...
    #数据库文件绝句路径
    global DB_FILE_PATH
    DB_FILE_PATH = 'D:\\SQLite\\hongten.db'
    #数据库表名称
    global TABLE_NAME
    TABLE_NAME = 'student'
    #是否打印sql
