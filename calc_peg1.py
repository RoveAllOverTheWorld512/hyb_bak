...
def write_xls(xlsfile,data):
    ...
    heading_xf = NamedStyle(name="title")
    heading_xf.font = Font(bold=True,size=10)
    heading_xf.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    wb.add_named_style(heading_xf)
    
    flt_xf = NamedStyle(name="flt")
    flt_xf.font = Font(bold=False,size=10)
    flt_xf.alignment = Alignment(horizontal="right", vertical="center", wrap_text=True)
    flt_xf.number_format = '#0.00'
    wb.add_named_style(flt_xf)

    heading = ws['A1:AE1']
    for cell in heading[0] :
        cell.style = heading_xf
...
#        sys.exit(3)
    ...
    if bklb == 'zd' :
        ...
        if len(pegxls)== 0:
            ...
            pegxls = os.path.join(getpath(),'zd_'+zxgwjm[0]+"_peg.xlsx")
    else :
        pegxls = os.path.join(getpath(),bkxx+"_peg.xlsx")
