...
#    browser = webdriver.PhantomJS()
    ...
    url='http://stockdata.stock.hexun.com/2008/ggqw.aspx?page=%d&stockid=%s' % (pg,gpdm)
    browser.get(url)
    time.sleep(3)

    elem=browser.find_element_by_class_name('box19')
    zys=elem.find_element_by_tag_name('strong')
    pgs=int(1/eval(zys.text))
    
    for pg in range(1,pgs+1):
        url='http://stockdata.stock.hexun.com/2008/ggqw.aspx?page=%d&stockid=%s' % (pg,gpdm)
        browser.get(url)
        
        elem = browser.find_element_by_id('zaiyaocontent')
    
        html=elem.get_attribute("outerHTML")
        tbl=pq(html)
        
        for i in range(len(tbl('tr'))):
            ...
            #pyquery 提取网页内容
            #https://www.jianshu.com/p/2435e2aa30ee
            
            pdfurl=row('a').eq(1).attr('href')
            data.append([pg,bt,rq,pdfurl]) 


    browser.quit()
    columns=["页码","标题","日期",'PDF地址']
    df = pd.DataFrame(data,columns=columns)

    xlsfn=dldir+'\\公告%s.xlsx' % gpdm

    writer=pd.ExcelWriter(xlsfn,engine='xlsxwriter')

    df.to_excel(writer, sheet_name='所有公告',index=False)   
