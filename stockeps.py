...
def dl_ths_xls(prefix):
    ...
    for j in range(sn,qn+1):
        ...
        kw = readkey(config, prefix + 'kw' + str(j))
        sele = readkey(config, prefix + 'sl' + str(j))
        newfn = readkey(config, prefix + 'fn' + str(j))
        newfn = os.path.join(ddir,newfn)
...
基于pandas进行数据预处理
http://blog.csdn.net/u014400239/article/details/70846634
'''
